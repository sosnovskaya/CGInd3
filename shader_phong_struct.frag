...
void main(){
	...
	for(int i = 0; i < cnt; ++i){
		...
		float spotEffect = dot(spotDir, lightDir); // cos угла отлконения
		...
		float spot;
			if(l[i].spot_cutoff == -1)
				...
				spot= float(spotEffect > l[i].spot_cutoff); // 0  и  1 - точка под свет
		
		
		if(l[i].spot_exp != 0)
			spotEffect = max(pow(spotEffect, l[i].spot_exp), 0.0); // из косинуса превращается в затухание от центра
		else
			spotEffect = 1.0;
